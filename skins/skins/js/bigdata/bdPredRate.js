			...
			$.fn.postSubmit(GlobalParma.dataUrl,JSON.stringify(jsonData), function(data){
				if(data.success){
					if(data.data&&data.data.length>0){
						...
						//图表数据
						var datas = new Array();
						var markPointDatas = new Array();
						for(var i=0;i<data.data.length;i++){
							var x = data.data[i].PRED_CYCLE;
							var y = parseFloat(data.data[i].ACCURACY);
							xData.push(x);
							datas.push({
								name:x,
								value:y
						...
						cur.option.xAxis.data = xData;
						cur.option.series[0].data = datas;
						cur.option.series[1].data = datas;
						cur.option.series[0].markPoint.data = markPointDatas;
