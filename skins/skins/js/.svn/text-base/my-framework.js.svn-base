...
L.FlexGrid = function(listId, appUrl, usingOdata) {
	...
	this.defaults = {
		...
		drawCallback : tableDrawCallback, // 表单加载完成回调
		format: tableRowFormat,
		createdRow : function(){} //Row created callback add by shanleiyang 
	};
	this.oTable = null;
};
...
L.FlexGrid.prototype = {
	...
	init : function(options) {// 列信息数组
		...
		_that.oTable = $('#' + this.listId).DataTable({
			...
			"serverSide" : this.defaults.serverSide,//true代表后台处理分页，false代表前台处理分页
			...
			"createdRow" : this.defaults.createdRow //Row created callback add by shanleiyang
		});
		...
		$.each(thead, function(trindex, trelement) {
		  $.each(trelement, function(thindex, thelement) {
		    ...
		    if(thelement != 0) {
				...
				if($(thelement).attr("data-field") && $(thelement).attr("data-field") == 'columnSeqNo') {
					columnMap["data"]='columnSeqNo';
					columnMap['bSortable'] = false;
					columnMap['columnName'] = 'columnSeqNo';
					columnList[thindex] = columnMap;
					return true;//类似continue
				}else if($(thelement).attr("data-field")) {
	...
	retrieveData: function(sSource, aoData, fnCallback, oSettings){
		...
		if (_that.defaults.ordering) {			
			//排序信息
			//如果有排序信息
			if(aoData[2] && (aoData[2].value).length > 0){
				//单列排序
		...
		var draw = aoData[0] ? aoData[0].value : 1;
		//var length = initData.length ? initData.length : 0;
		var total = _that.total;
		var allData = _that.allData;
		if(_that.columnList.length > 0 && _that.columnList[0]['columnName'] == 'columnSeqNo' && initData && initData.length > 0) {
			//添加序号列 columnSeqNo
			for(var di=0; di < initData.length; di++) {
				allData.data[di]["columnSeqNo"] = di + 1;
				initData[di]["columnSeqNo"] = di + 1;
			}
		}
		var json = {
			"draw": draw,
	...
	getSelectedData: function(){
		var _that = this;
		var checkBoxRows = $(".table>tbody>tr>td input[type='checkbox']");
		var radioRows = $(".table>tbody>tr>td input[type='radio']");
		var selectedData = [];
		checkBoxRows.each(function(i, n){
			if($(this).prop("checked"))
			{
				var data = _that.oTable.row($(this).parents("tr")).data();
				selectedData.push(data);
			}
		});
		radioRows.each(function(i, n){
			if($(this).prop("checked"))
			{
				var data = _that.oTable.row($(this).parents("tr")).data();
				selectedData.push(data);
...
L.EditGrid.prototype = {
		...
		getSaveDate:function(){
			...
			for(var i = 0,datalen=data.length ; i < datalen; i++) {
				...
				var dataXH = data[i]["xh"] ? data[i]["xh"] : 0;//序号默认为0
				data[i]["xh"] = dataXH;
				var bo = dictCode&&itemCode&&itemValue&&dataState!=2 ? true : false;
				if(bo) {
			...
			$ths.each(function(i, th){
				...
				switch(editor) {
					...
					case "date":
						...
						break;
					case "datetime":
						col["editor"] = Slick.Editors.MonthDateTimeEditor;
						break;
					case "time":
						col["editor"] = Slick.Editors.MonthTimeEditor;
						break;
					default:
						break;
				}
...
function isNumber(value) {
	var reg = /^[0-9]*$/;
	if(!reg.test(value)) {
    	return {valid: false, msg: L.EditGrid.locale['isNumber']};
	}
...
function isFloat(value){
	var regx = /^(-?\d+)(\.\d+)?$/;
	if(!reg.test(value)) {
    	return {valid: false, msg: L.EditGrid.locale['isFloat']};
	}
	return {valid: true, msg: null};
}
//字符串
function isString(value){
	var regx = /^[\u4E00-\u9FA5\uf900-\ufa2d\w\.\s]+$/;
	if(!regx.test(value)){
		return {valid: false,msg:L.EditGrid.locale['isString']};
	}
	return {valid: true, msg: null};
}
//邮箱校验规则
function isEmail(value){
	...
	if(!regx.test(value)){
		return {valid: false, msg: L.EditGrid.locale['isEmail'] };
	}
	return {valid: true, msg: null};
}
...
function isUrl(value){
	var regx = /^(\w+:\/\/)?\w+(\.\w+)+.*$/;
	if(!regx.test(value)){
		return {valid: false, msg: L.EditGrid.locale['isUrl'] };
	}
	return {valid: true, msg: null};
}

//空值校验
function isEmpty(value, dataContext, columnDef) {
	if(value == null || value == undefined || !value.length) {
		return {valid: false, msg: '第'+(columnDef.item.rowId * 1 + 1)+'行"'+columnDef.column.name+'"不能为空!'};
	}
	return {valid: true, msg: null};
}
