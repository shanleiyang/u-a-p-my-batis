...
function changeByMap(countyCode){
	...
	window.parent.changeByChildren("/home/bigdata/showpage/guzhangyc/guzhangyc_qx.jsp");
	window.location.href = context+"/bigdata/showpage/guzhangyc/guzhangyc_qx.jsp?cityCode="+GlobalParma.cityCode+"&countyCode="+countyCode+"&year="+year+"&month="+month;
}
/*到货批次钻取到到货批次详情页面*/
function gotoDHPCXQ(batchNo){
	...
	window.location.href = context+"/bigdata/showpage/daohuopc/daohuopicixq.jsp?provinceNo="+provinceNo+"&year="+year+"&month="+month+"&bidBatchNo="+batchNo+"&cityCode="+cityCode;
}
function query(){
...
<uap:asset path="jquery.js,echarts.min.js,/map/bigdatamap.js"/>
...
var vm = new Vue({
	...
	computed:{
		...
		malfunctionRateMonth:function(){return new MalfunctionRateMonth($("#malfunctionRateMonth")[0],'dsgzyc_gzlchart');},
		malfunctionRateTotal: function(){return new MalfunctionRate($("#malfunctionRateTotal")[0],'dsgzyc_gzlchart');},
		bigdatamap:function(){
			return new BigDataMap($('#faultcitymap')[0],"dsgz_map");//从全局变量中获取mapType
		},
	...
	mounted: function(){
		cityFault.addObserve(this.bigdatamap);
		cityFault.addObserve(this.malfunctionRateMonth);
		cityFault.addObserve(this.malfunctionRateTotal);
	},
	methods:{
		initChart:function(){
